document.addEventListener("DOMContentLoaded", () => {
    ...
    const compliteContainer = document.querySelector(".complite_task");
    ...
    class TaskManeger {
        ...
        submitForm() {
            ...
            localStorage.setItem(obj.task_name, JSON.stringify(obj));
            return obj;
        }
    }
    ...
    if(localStorage.length > 0) {
        for(value of Object.values(localStorage)) { 
            let item = JSON.parse(value);
            if(item.complite) {
                let task = new Task(item, compliteContainer);
                task.create();
                task.div.querySelector(".task-second-container").remove();
                task.div.querySelectorAll(".sub_checkbox").forEach(item => {
                    item.remove();
                })
                task.div.style.textAlign = "left";
                task.put();

            }  else {
                let task = new Task(item, taskContainer);
                task.create();
                task.put();
            }
    ...
    taskContainer.addEventListener("click", (ev) => {
        ...
        if (ev.target.classList.contains("complite_button")) {
            let localName = parent.querySelector("h3").textContent;
            let localItem = JSON.parse(localStorage.getItem(localName));
            localItem.complite = true;
            ...
            let nameKey = ev.target.parentElement.previousElementSibling.querySelector("h3").textContent;
